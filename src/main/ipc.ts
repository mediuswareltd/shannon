...
export function registerIpc(): void {
  ...
  ipcMain.handle('activity:list', async (_e, opts?: { refresh?: boolean }) => {
    try {
      ...
      if (!opts?.refresh && activityCache && Date.now() - activityCache.at < TTL_MS) {
        return activityCache.rows
      }
      const collected = await collectEvents(accounts)
      ...
      for (const c of collected) {
        ...
        if (c.event.type === 'PushEvent') {
          ...
          if (inlined.length > 0) {
            ...
            remoteEnrichUsed += 1
            try {
              const msgs = await withTimeout(enrichPushCommitMessages(c.event, token), ENRICH_TIMEOUT_MS)
  ...
  ipcMain.handle('repos:list', async () => {
    ...
    const merged = await collectRepos(accounts)
    return merged.map(({ accountLogin, repo }) => ({
      accountLogin,
      id: repo.id,
